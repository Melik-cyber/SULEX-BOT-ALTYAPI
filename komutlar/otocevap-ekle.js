...
let prefix = await db.fetch(`prefix.${message.guild.id}`) || fynx.prefix 
  ...
  .setDescription('<a:carpi:821416413138911272> Bu Komutu Kullanmak İçin `Sunucuyu Yönet` Yetkisi Lazım!')
  .setColor("RED");

                let mentionEmbed = new Discord.MessageEmbed()
                     .setDescription(`<a:carpi:821416413138911272> Lütfen \`komut\` - \`gönderilecek şey\` olarak kullanınız.\nÖrnek: \`${prefix}özel-komut-ekle minecraft-ip play.serveripniz.com\``)
                     .setColor('RED')
...
if (!message.member.hasPermission('MANAGE_GUILD')) return message.channel.send(embeddd)
 ...
 if(args[0] == 'yardım' || args[0] == 'bilgi') return message.channel.send('<a:carpi:821416413138911272> Botun varolan bir komudunu ekleyemezsin.')                   
  let gonderileceksey;
                     if (args.slice(1, 1000, args[1]).join(' ') === 'NONE') gonderileceksey = ''; 
                     else gonderileceksey = args.slice(1, 1000, args[1]).join(' '); 
                
                     let welcomeEmbed = new Discord.MessageEmbed()
                     .addField(`<a:mavitik:821416581216600135> Bu sunucuya özel komut eklendi.`, `\`${komut}\` yazıldığı zaman \`${gonderileceksey}\` olarak yanıt verecek.`)
                     .setColor('GREEN')
